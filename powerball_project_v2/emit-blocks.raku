...
sub MAIN(
    ...
    for @dates -> $d {
        if %by-date{$d}:exists('pb') {
            ...
            @json.push: { draw_date => $d, numbers => $r.nums, powerball => $r.pb,
                          multiplier => $r.mult.chars ?? $r.mult !! Nil, source => 'floridalottery',
                          jackpot_usd => Nil } if $emit eq 'json' or $emit eq 'both';
        }
        if %by-date{$d}:exists('dp') {
            my $r = %by-date{$d}{'dp'};
            say fmt-block($r.nums, $r.pb, $d, 'dp') if $emit eq 'blocks' or $emit eq 'both';
            @json.push: { draw_date => $d, numbers => $r.nums, powerball => $r.pb,
                          multiplier => Nil, source => 'floridalottery dp', jackpot_usd => Nil }
